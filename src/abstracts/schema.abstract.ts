...
import { ViewElement, ViewTab, ParsedContract } from '../interfaces';
...
export abstract class ViewSchema {
    ...
    public parserPage(contract: ParsedContract, page: AbstractPage): string {
        return '';
    }
    ...
    public validateProp(
        ...
        for (const type of types) {
            if (typeCheckers[type.trim()]?.(propValue)) return true;
        }

        throw new Error(
            `Invalid type for prop "${propKey}". Expected "${expectedType}", but received "${typeof propValue}".`,
        );
    }
}
