...
<template>\n    <Vueform ref="form$">\n`;
        ...
        for (const [key, component] of Object.entries(components)) {
            ...
            if (!schemaComponent)
                throw new Error(
                    `Component type "${component.type}" not found in schema.`,
                );
            ...
            for (const [propKey, propValue] of Object.entries(
                ...
                if (propKey === 'name')
                    throw new Error(
                        `Prop "name" property must not be set to prop.`,
                    );
                ...
                if (props && props[propKey]) {
                    ...
                    this.validateProp(
                        propKey,
                        propValue,
                        this.genericAttrs[propKey],
                    );
                } else {
                    throw new Error(
                        `Prop "${propKey}" is not valid for component type "${component.type}".`,
                    );
                }
